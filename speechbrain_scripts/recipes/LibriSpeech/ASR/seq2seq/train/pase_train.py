#!/usr/bin/env python3
import numpy as np
from asteroid.filterbanks.multiphase_gammatone_fb import MultiphaseGammatoneFB
...
#    features  = compute_cw(features)
    ...
    return features
gammatone_filter = Gammatonegram(hop_length=160, sr=16000).cuda()
# Define training procedure
class PASE(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if "gammatone" in self.considered_workers : 
            ...
            #print(gammafeats)
            #print(torch.min(gammafeats))
            #print(torch.max(gammafeats))
            #print(torch.mean(gammafeats))
            #print(torch.std(gammafeats))
            gammafeats = torch.reshape(gammafeats, (feats.size()[0], -1,64))

            #gammafeats = self.modules.gamma_normalizer(gammafeats, wav_lens)
            #print(gammafeats)
    ...
    def compute_objectives(self, predictions, batch,other_targets, stage):
        ...
        if len(exoworkers)>0 : 
            ...
            for ind,worker in enumerate(exoworkers) : 
                if worker not in ["melfs", "mfcc", "gammatone"] : 
                    worker_values = [workers_targets_values[i][ind] for i in range(Nb)]
        ...
        for worker in self.considered_workers : 
            ...
            if len(self.workers_loss_recap[worker])==100 : 
                ...
                print(f" for this worker : {worker} ; mean loss is {np.mean(self.workers_loss_recap[worker])}")
    ...
    def fit_batch(self, batch):
        ...
        if self.hparams.batch_counter  % 1000 ==0 : 
...
def dataio_prepare(hparams):
    ...
    if len(needed_workers)>0 : 
        print(needed_workers) 
        @sb.utils.data_pipeline.takes(needed_workers)
        @sb.utils.data_pipeline.provides("workers_targets")
        def float_list(*args):
            outs = []
            for arg in args : 
                g=arg.split()
                f=[float(x) for x in g]
                f=torch.tensor(f)
                outs.append(f)
            return outs
        sb.dataio.dataset.add_dynamic_item(datasets, audio_pipeline)

        sb.dataio.dataset.add_dynamic_item(datasets, float_list)
        # 4. set output:
